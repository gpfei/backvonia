...
//! Different limits apply based on purchase tier and endpoint.

use crate::{
    error::{ApiError, Result},
    middleware::auth::IAPIdentity,
...
pub fn rate_limit_middleware(
    ...
    move |request: Request, next: Next| {
        ...
        Box::pin(async move {
            ...
            let identity = request.extensions().get::<IAPIdentity>().ok_or_else(|| {
                ApiError::Internal(anyhow::anyhow!(
                    "Rate limit middleware requires auth middleware"
                ))
            })?;

            // Determine rate limit based on tier
            let limit = match identity.purchase_tier {
                PurchaseTier::Free => config.free_tier_rpm,
                PurchaseTier::Pro => config.pro_tier_rpm,
            };
            ...
            if !allowed {
                warn!(
                    "Rate limit exceeded for identity: {} (tier: {:?})",
                    identity.purchase_identity, identity.purchase_tier
                );
                return Err(ApiError::RateLimitExceeded);
            }

            debug!(
                "Rate limit check passed for identity: {} (tier: {:?})",
                identity.purchase_identity, identity.purchase_tier
            );
